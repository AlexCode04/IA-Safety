...
def reasoning_before_decision(text: str) -> str:
    ...
    return text.partition("<DECISION>")[0].strip()
...
class AgentOutput:
    ...
    def prefix_until_decision(self) -> str:
        if self.activation_prefix is not None:
            return self.activation_prefix
        reasoning = reasoning_before_decision(self.reasoning)
        return f"REASONING:\n{reasoning}{DECISION_MARKER}"
