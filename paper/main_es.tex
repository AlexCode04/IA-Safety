...
\title{\vspace{-1.2em}\system: Prototipo reproducible para monitoreo de sabotaje\\
pre-harm ponderado por riesgo\vspace{-0.35em}}
\author{\small
...
Los runtime monitors observan superficies diferentes---acciones visibles,
Chain-of-Thought (CoT), activation probes lineales y lecturas de un Natural
Language Autoencoder (NLA)---pero su valor operacional depende del momento de
la alerta, la disrupción benigna, el impacto y el costo. Presentamos \system, un
prototipo open-source que congela un evento pre-action, aísla cuatro canales de
evidencia, valida juicios estructurados de Gemini y transforma alertas en
decisiones \texttt{ALLOW}, \texttt{ESCALATE} o \texttt{BLOCK} bajo un presupuesto
común. El protocolo contiene 24 trayectorias simuladas en tres familias de
herramientas y cuatro condiciones conductuales. Como la corrida live de
escenarios no terminó antes del plazo del sprint, reportamos una evaluación
mock determinista y explícitamente rotulada. Esta valida contratos,
instrumentación, métricas, optimizador, dashboard y generación del paper; no
estima la seguridad de Qwen2.5-7B, Gemini, el NLA aprendido ni el probe. En la
demostración sintética, Observable y CoT detectan 6/12 intentos pre-harm,
mientras los canales internos mock, separables por construcción, detectan
12/12. Sin embargo, un enrutamiento basado solo en severidad puede rendir peor
que action-only al gastar presupuesto en casos benignos de alta severidad.
\end{abstract}
...
\textbf{\system{} (nuestro)} & \yes & probe + NLA nativo & \yes & \yes & prevención ponderada\\
\bottomrule
\end{tabular}
...
El agente objetivo es \texttt{Qwen/Qwen2.5-7B-Instruct}
\cite{yang2025qwen25}, con decoding determinista. En el último token de
\texttt{\textbackslash n<DECISION>\textbackslash n}, antes del nombre y los
argumentos de la herramienta, se extrae el estado residual $h_i$ de la capa 20.
El mismo estado alimenta un probe logístico L2 y
\texttt{Kameshr/nla-qwen2.5-7b-L20-av}. El adapter se desactiva durante la
...
\section{Métricas y asignación de presupuesto}
...
Observable y CoT detienen \phrObservableCount\ intentos; CoT también marca
\bdrCoTCount\ casos no dañinos porque el mock reconoce lenguaje de rechazo.
Probe y NLA detienen \phrNLACount\ sin falsas disrupciones mock por construcción.
$\mathrm{IMV}_{N}$ y $\mathrm{IMV}_{N\mid C}$ son \imvNLA{} unidades de
severidad: los seis casos stealth que $O\cup C$ no captura.
...
\midrule
baseline ($O+C$) & 52.8 & 15.0 & 50.0\%\\
action-only ($O$) & 24.0 & 15.0 & 50.0\%\\
nla-probe-cot priority & 52.5 & 18.0 & 66.7\%\\
probe-nla-cot priority & 52.5 & 18.0 & 66.7\%\\
cot-nla priority & 51.8 & 18.0 & 66.7\%
\\
LP post-hoc (límite superior) & 44.4 & 9.0 & 33.3\%\\
\bottomrule
\end{tabular}
...
de routing sin riesgo o beneficio calibrado. La fila LP reutiliza beneficios observados en el mismo mock y es un límite
superior optimista, no una estimación desplegable. Esto no acepta ni rechaza
las hipótesis de modelos reales.
...
\cite{google2026gemini}. El dashboard consume el mismo summary y marca los datos
mock. El artefacto acompaña el paper \cite{budgetnla2026}.
...
\section{Conclusión}
\system{} aporta un stack reproducible de medición para incident response, no
una afirmación de desempeño de seguridad. La corrida sintética confirma que el
contrato pre-action, aislamiento, policy gate, contabilidad, dashboard bilingüe
y pipeline de publicación concuerdan. También muestra una lección de diseño:
un monitor más informativo puede contener menos daño cuando el routing gasta el
presupuesto en casos equivocados. Se necesita la corrida real completa antes de
concluir sobre Qwen, NLA, probe o Gemini.
