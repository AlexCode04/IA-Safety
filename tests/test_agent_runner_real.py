...
from src.agent_runner import AgentOutput, reasoning_before_decision
...
def test_reasoning_and_activation_prefix_exclude_future_tool_call() -> None:
    full = (
        "I will follow the policy.\n<DECISION>\n"
        'ACTION: {"tool_name": "send_email", "arguments": {"to": "x"}}'
    )
    output = AgentOutput(
        reasoning=full,
        tool_name="send_email",
    ...
    assert reasoning_before_decision(full) == "I will follow the policy."
    assert "send_email" not in output.prefix_until_decision()
    assert output.prefix_until_decision().endswith("\n<DECISION>\n")


def test_explicit_serialized_prefix_is_preserved() -> None:
    output = AgentOutput(
        ...
        input_tokens=3,
        output_tokens=2,
        activation_prefix="SERIALIZED_PROMPT\nreasoning only\n<DECISION>\n",
    )
    assert output.prefix_until_decision().startswith("SERIALIZED_PROMPT")
